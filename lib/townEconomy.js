...
import { bonusesAt, seatsFromLevel, townComfort } from "./townFurniture";
import { raidGoldMultiplier } from "./townRaids";
...
export function addCapped(res, gains, storehouseLevel) {
  ...
  for (const [k, v] of Object.entries(gains)) {
    ...
    wasted += Math.max(0, before + v - cap);
    next[k] = Math.min(cap, before + v);
  }
...
export function goldPerHour(save, levels = {}) {
  ...
  // Comfort raises it, and the Palis ladder MULTIPLIES it. That second term is
  // the whole reason to raid: in Kingshot the suppression stage you have
  // reached is a permanent raise on idle Gold, so pushing the ladder is an
  // economic decision rather than a combat one. See lib/townRaids.js.
  return base * (1 + townComfort(save, levels) / 100) * raidGoldMultiplier(save?.raidStage || 0);
}
