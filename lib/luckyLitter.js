...
export function spin(wheelId, pity = 0, rng = Math.random) {
  ...
  // The common slot gives a VILLAGER — a real cat with a real name who moves
  // into a cottage and works. It used to hand back a thing called "an ordinary
  // cat", which is a miserable thing to receive from a wheel you paid to spin,
  // and it was also wrong: population is the resource that gates every building
  // in the town. The caller resolves the actual cat from the pool.
  if (rarity === "common") {
    return { hero: null, villager: true, rarity, shards: w.shards.common, pity: false };
  }
...
// ---------------------------------------------------------------------------
//  THE DAILY KEY ALLOWANCE — the reason a free player has a roster at all
//
//  Two days in, a player had ONE hero. In Kingshot, two days in, you have six:
//  an SR, two Rs and three commons. The difference is not the odds, it is the
//  VOLUME — Kingshot hands out free recruitment every single day and its guides
//  all say the same thing, never skip the daily free pulls.
//
//  So keys arrive on a clock, whether or not the player does anything:
//
//    · a SILVER key every 8 hours — three a day, the everyday wheel
//    · a GOLD key every 24 hours — the good one, held to one a day
//
//  Both accrue while you are away and both CAP, so the faucet cannot be farmed
//  by disappearing for a month and it still rewards coming back tomorrow. The
//  cap is the retention mechanism; the rate is the generosity.
// ---------------------------------------------------------------------------

export const KEY_FAUCET = {
  silver: { everyMs: 8 * 3_600_000, cap: 6 },
  gold: { everyMs: 24 * 3_600_000, cap: 2 },
};

/** How many free keys have accrued since `since`, and when the clock should be
 *  reset to. Returns whole keys only — the remainder stays on the clock rather
 *  than being rounded away, so a player who checks in every seven hours is not
 *  quietly earning nothing. */
export function keysAccrued(kind, since, now = Date.now(), held = 0) {
  const f = KEY_FAUCET[kind];
  if (!f || !since) return { got: 0, at: now };
  const whole = Math.floor((now - since) / f.everyMs);
  if (whole < 1) return { got: 0, at: since };
  // The cap is on what the FAUCET may hand you while away, not on what you may
  // hold — a player who buys ten keys does not lose them.
  const got = Math.max(0, Math.min(whole, f.cap - Math.min(held, f.cap)));
  return { got, at: since + whole * f.everyMs };
}

/** Milliseconds until the next free key of this kind. */
export function nextKeyIn(kind, since, now = Date.now()) {
  const f = KEY_FAUCET[kind];
  if (!f || !since) return f ? f.everyMs : 0;
  const elapsed = (now - since) % f.everyMs;
  return Math.max(0, f.everyMs - elapsed);
}
