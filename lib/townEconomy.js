...
/** CAT VILLAGERS — the town's population, the cats that go inside buildings
 *  and do the work.
 *
 *  The Cat Hall caps how many can be on shift at once, exactly the way
 *  Kingshot's Town Center caps everything: one building that every route of
 *  progress has to come back through. Not the Nap House — that is where tired
 *  cats sleep, which is a different job.
 *
 *  Two at Cat Hall 1, one more per level. */
export const villagerCap = (hallLevel) => hallLevel + 1;

/** Back-compat alias; call villagerCap. */
export const workerCap = villagerCap;
...
export const RESOURCE_USES = {
  ...
  // Two routes to more worker spots, and the text has to make the difference
  // obvious: raising the Nap House earns them, Golden Fish buys them.
  wood: ["The backbone of every construction", "Raising the Cat Hall, which raises the villager cap"],
 ...
 *  purchase now (workerSlotPriceUsd). */
