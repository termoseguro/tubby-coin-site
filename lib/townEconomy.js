...
/** A building's output multiplier from the cats standing in it. Zero cats
 *  still produces something — the town should never fully stall — but the
 *  difference is visible enough to make assignment matter. */
export function staffing(catsHere) {
  return [0.45, 0.75, 1, 1.2][Math.min(3, Math.max(0, catsHere))];
}
...
export function effectiveRate(buildingId, level, { catsHere = 0, starving = false } = {}) {
  const base = ratePerHour(buildingId, level);
  return base * staffing(catsHere) * (starving ? STARVING_MULTIPLIER : 1);
}
