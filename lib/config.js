...
export const config = {
  ...
  //
  //   care   the Tubby Cares fund — goods for children's shelters in Rio.
  ...
  links: {
    ...
    deliveryLog: "#", // Tubby Cares delivery log — receipts, CNPJs and photos
  },
  ...
  wallets: {
    care: "{{WALLET_CARE}}", // fee wallet A — Tubby Cares (shelter supplies)
  ...
  liveData: {
    ...
    demoFundedSol: 3.2, // demo basket — kept UNDER milestones[0] so the preview reads as "filling up"
  },
  ...
  //
  // ⚠ `items` IS A COMMITMENT. Set every line from a REAL quote before launch,
  // never from an estimate — the seeds below assume roughly R$700-800 per SOL
  // and WILL be wrong by launch day. Under-promise here, over-deliver in the
  // photos. Anything already delivered moves to `care.deliveries` below.
  milestones: [
    { sol: 5, emoji: "🍼", label: "the first run", items: "400 diapers + 80 tins of formula" },
    { sol: 15, emoji: "🥫", label: "a full month", items: "a month of diapers and formula for one shelter, plus 30 food baskets" },
    { sol: 40, emoji: "💊", label: "three shelters", items: "three shelters stocked, plus a first-aid and medicine restock" },
    { sol: 100, emoji: "🏠", label: "the long haul", items: "a year of diapers for one shelter, bought in one go" },
  ],
  ...
  care: {
    cause: "children's shelters in Rio de Janeiro",
    ...
    drives: [
      {
        ...
        org: "a children's therapy centre in Rio de Janeiro",
        items: "60+ plush cats, plus diapers and hygiene supplies",
        note:
          "Bought at the supermarket, wheeled in by hand, handed over the same afternoon. " +
          "It is also the drive that exposed the gap: toys are the donation everybody makes, " +
          "and the shelves were still short of the boring things. That is what the coin buys now.",
        // No people in frame — safe to publish anywhere, and the strongest
        // evidence anyway: you can count the goods.
        photos: ["/cares/drive-01-supplies.webp"],
        // The same drive, showing the children who received them. Faces already
        // covered. Governed by publishPeoplePhotos below.
        people: [
    ...
    //
    // ⚠ LEAVE THIS FALSE until the institution has given WRITTEN permission for
    // public online use. Covering the faces was the right instinct and it is
    // not the whole job: the wheelchairs, the therapy room and the equipment
    // still disclose health status and identify which institution a specific
    // child attends. ECA arts. 17 and 18 (image and dignity) and art. 143
    // (identifying a minor), plus LGPD art. 11 — health is sensitive personal
    // data and needs its own, explicit consent.
    //
    // The supplies photo carries the story without any of that exposure, which
    // is why it is the one that runs by default.
    publishPeoplePhotos: false,
  },
