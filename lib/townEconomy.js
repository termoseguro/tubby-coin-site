...
export function storeCap(storehouseLevel, res, hallLevel = 1) {
  // THE CAP HAS TO GROW WITH THE TOWN, NOT ONLY WITH THE STOREHOUSE.
  //
  // This was a flat 4,000 until a Storehouse existed — and the Storehouse
  // unlocks at Cat Hall 9. Cat Hall 4 -> 5 costs 5,614 Wood. So every town in
  // the game hit a wall at Cat Hall 4 that no amount of playing could pass:
  // the stockpile physically could not hold the price of the next upgrade.
  // Three deadlocks of this exact shape have now been found by simulation, all
  // of them invisible to a rules-only check.
  //
  // So the Cat Hall carries the base capacity — the town centre gates
  // everything else, and this is no exception — and the Storehouse multiplies
  // it, at double weight, which is what makes it worth building at all. Tuned
  // to sit around 2.5x the next Cat Hall upgrade: loose enough that the ladder
  // is always payable, tight enough that a day away still overflows.
  const rank = Math.max(1, hallLevel) + Math.max(0, storehouseLevel) * 2;
  const base = 1200 * Math.pow(rank, 1.85);
...
export function upgradeCostFor(buildingId, level) {
  ...
  // NEVER CHARGE FOR A RESOURCE THE LADDER HAS NOT UNLOCKED.
  //
  // A brand new game was UNWINNABLE and it took a simulation to find. The
  // opening costs 360 Fish for Cat Hall 1 -> 2; the Kitchen — the only source
  // of Fish there is — unlocks AT Cat Hall 2. A new town started with 250 Fish
  // and no way on earth to reach 360. Every single new player was stuck on the
  // first move, forever, with no message saying why.
  //
  // Fixing that one number would have fixed that one bug. This fixes the class
  // of bug: the cost is filtered against what the town can actually produce at
  // this Cat Hall level, so the ladder can be re-ordered freely and can never
  // ask for something that does not exist yet. `stone` and `catnip` are
  // already gated by level, and this makes the gate real rather than a
  // coincidence of the two ladders happening to line up.
  const SOURCE = { fish: "kitchen", stone: "quarry", catnip: "garden", treats: "treats" };
  for (const [res, source] of Object.entries(SOURCE)) {
    if (cost[res] == null) continue;

    // CASE 1 — a producer cannot cost the thing it is the only source of.
    // The Kitchen's own construction asked for 150 Fish, and the Kitchen is
    // where Fish comes from. A town that had spent its opening Fish could
    // never build one, and therefore could never have Fish again.
    if (buildingId === source && level < 1) {
      delete cost[res];
      continue;
    }

    // CASE 2 — the Cat Hall is the only building that can be upgraded before
    // its producers exist, and that is provable rather than assumed:
    // maxLevelFor caps every other building at the Cat Hall's level, so
    // upgrading one requires level < hallLevel, hence hallLevel >= 2, hence
    // the Kitchen is already standing. Cat Hall 1 -> 2 asked for 360 Fish and
    // the Kitchen unlocks AT Cat Hall 2.
    if (buildingId === "hall" && !isUnlocked(source, L)) delete cost[res];
  }
  return cost;
}
 ...
 *
 *  THIS IS THE GAME'S CLOCK AND IT WAS SET TO MINUTES. The old curve finished a
 *  Cat Hall 30 in under two hours and an ordinary building in thirty minutes,
 *  with a comment admitting it was a play-testing shortcut. The consequence was
 *  not just "too fast": simulation showed the town's only bottleneck was
 *  RESOURCES, so extra builders had nothing to do, and builders are the genre's
 *  most-bought product. A $150 player finished thirty days in exactly the same
 *  place as a $10 one.
 *
 *  Real timers put the two constraints back in tension — you are always either
 *  waiting on materials or waiting on a builder — which is what makes a second
 *  builder worth buying and a rush worth spending on.
 *
 *      Cat Hall  L1  2m   L5  1.2h   L10  5.3h   L20  1.2d   L30  2.9d
 *      Others    L1  1m   L5  0.6h   L10  2.6h   L20  14h
 *
 *  ⚠ The server owns this clock. Client-side it is advisory — security.md §1. */
export function buildSecondsFor(buildingId, level) {
  const k = buildingId === "hall" ? 2 : 1;
  return Math.round(60 * k * Math.pow(Math.max(1, level), 2.2));
}
...
export function addCapped(res, gains, storehouseLevel, hallLevel = 1) {
  const next = { ...res };
  let wasted = 0;
  for (const [k, v] of Object.entries(gains)) {
    if (!v) continue;
    const cap = storeCap(storehouseLevel, k, hallLevel);
...
// ---------------------------------------------------------------------------
//  THREE CAPS, AND EACH ONE HAS AN OWNER
//
//  This is the part that was wrong, and it was wrong by omission. Kingshot has
//  THREE limits on cats at work and I had implemented two, which left the two
//  survivors unrelated to each other and impossible to read:
//
//    1. HOW MANY CATS EXIST      → Cat Cottages          (Houses, up to 32)
//    2. HOW MANY MAY BE AT WORK  → the CAT HALL          ← this one was missing
//    3. HOW MANY IN THIS BUILDING→ that building's level (1 / 4 / 7)
//
//  Kingshot states the second plainly: "The Town Centre level determines the
//  maximum number of survivors that can be assigned as workers to these
//  buildings." Without it a player sees a bed count and a seat count that never
//  meet, and no single thing to raise to fix it. With it, every "no" has one
//  named answer: more cats → a cottage; more of them working → the Cat Hall;
//  more in THIS building → this building.
//
//  ⚠ UNCONFIRMED: the per-level numbers are not published anywhere I could find
//  — only the rule that the Town Center sets the cap. This curve is ours: two
//  at Cat Hall 1, one more per level, and it stops at 24 so the eight cottages
//  still house more than the town can employ, which is what keeps the "at home
//  earning Gold" side of the choice alive.
// ---------------------------------------------------------------------------

export const MAX_AT_WORK = 24;

/** How many cats the Cat Hall will let work at once. */
export const workersAllowed = (hallLevel = 1) =>
  Math.min(MAX_AT_WORK, 1 + Math.max(1, hallLevel));
...
export function goldPerHour(save, levels = {}) {
  ...
  // Comfort raises it, and that is all. The Long Alley used to multiply this
  // number too, which meant a player who never opened the hero screen had
  // their buildings quietly throttled by it. The Alley keeps its own purse
  // now — alleyPending() in lib/conquest.js — the way Kingshot does it.
  return base * (1 + townComfort(save, levels) / 100);
}
 ...
 *
 *  A BUILDING THAT DOES NOT EXIST HAS NO PLACES. The `max(1, …)` floor used to
 *  apply at every level including zero, so an empty plot advertised one seat —
 *  and everything downstream believed it. Auto-assign sent cats to buildings
 *  that had not been built, where `ratePerHour(id, 0)` is exactly 0, so the
 *  button reported "5 cats put to work" and nothing whatsoever happened. The
 *  floor belongs to built buildings only. */
export const slotsIn = (bought = 0, level = 1) =>
  level < 1 ? 0 : Math.min(MAX_PER_BUILDING, Math.max(1, seatsFromLevel(level)) + bought);
