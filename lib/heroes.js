...
//  FACES
//
//  Every hero gets a DIFFERENT cat. The first version indexed the pool with a
//  modulo, which quietly gave twelve Mythic heroes the same three faces — and a
//  roster where four cards share a portrait does not read as a roster, it reads
//  as a bug. So the assignment is a one-pass claim: walk the heroes in a fixed
//  order and take the next unclaimed cat, preferring one of the same rarity so
//  a Mythic hero wears a Mythic cat.
//
//  Computed once and cached, because it must be stable — a hero whose face
//  changes between renders is worse than a repeated one.
// ---------------------------------------------------------------------------

let faceCache = null;
let facePoolSize = -1;

function buildFaces(pool) {
  const taken = new Set();
  const out = {};
  // Rarest first, so the best heroes get first pick of the rarest art and the
  // commons absorb whatever is left over.
  const order = [...HEROES].sort(
    (a, b) =>
      ["common", "rare", "epic", "legendary", "mythic"].indexOf(b.rarity) -
      ["common", "rare", "epic", "legendary", "mythic"].indexOf(a.rarity)
  );
  for (const h of order) {
    const exact = pool.find((c) => c.rarity === h.rarity && !taken.has(c.art));
    const any = exact || pool.find((c) => !taken.has(c.art));
    if (!any) break;
    taken.add(any.art);
    out[h.id] = any.art;
  }
...
export function heroArt(heroId, pool) {
  ...
  if (faceCache === null || facePoolSize !== pool.length) {
    faceCache = buildFaces(pool);
    facePoolSize = pool.length;
  }
  return faceCache[heroId] || null;
}
