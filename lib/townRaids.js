...
import { catPower } from "./townEconomy";
...
export function townPower(save, levels = {}) {
  let power = 0;
...
export function resolveRaid(save, levels, stage) {
  const town = townPower(save, levels);
  const palis = palisPower(stage);
  const chance = winChance(town, palis);
  const won = Math.random() < chance;

  if (won) {
    return { won, town, palis, chance, loot: raidLoot(stage), hurt: hurtList(save, 0.25) };
  }
  ...
  return {
    ...
    palis,
    chance,
    loot: { coin: Math.round(raidPurse(stage) * 0.3) },
    hurt: hurtList(save, 0.6),
  };
}
