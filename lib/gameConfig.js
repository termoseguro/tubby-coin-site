...
export function rollRarity(forceMin, rng = Math.random) {
  // `rng` is injectable so the SERVER can pass a deterministic, seeded stream
  // and the roll becomes reproducible from a published seed — see
  // lib/server/gacha.js. On the client it stays Math.random and stays
  // advisory, because a client-side roll is a suggestion either way.
  const r = rng() * ODDS_TOTAL;
  let acc = 0;
