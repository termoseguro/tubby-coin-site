...
/** WHAT A NEW ACCOUNT IS GIVEN, FOR FREE, ON DAY ONE.
 *
 *  This used to be nothing: a new player spent every key they had and came away
 *  with one unascended Rare, which cannot clear the first Conquest stage and
 *  cannot staff a line-up of five. The hero half of the game was closed on
 *  arrival.
 *
 *  Kingshot does the opposite and it is not shy about it. Amadeus, a Legendary
 *  infantry hero, is available from server launch; Jabel, another Legendary, is
 *  the seven-day sign-in reward and "most players receive her in the first
 *  week"; Howard is the free SR infantry every beginner is told to build. A new
 *  Kingshot account is fielding a Legendary before it has decided whether it
 *  likes the game.
 *
 *  So: one Legendary immediately, and the four Rares — the production four, one
 *  per resource — which is a full line-up of five on the first screen. That is
 *  the grace period the hero game never had, and it is also the only honest way
 *  to teach a formation: you cannot learn front and back ranks with one cat.
 *
 *  The Legendary is chosen, not rolled. A random gift is a gift some players
 *  get and others resent. */
export const STARTER_HERO = "amadeu";
export const STARTER_RARES = ["biscuit", "timber", "pebble", "sprout"];

/** The day-one roster, as a save fragment. */
export function starterHeroes() {
  const heroes = { [STARTER_HERO]: { steps: 0, level: 1, shards: 0 } };
  for (const id of STARTER_RARES) heroes[id] = { steps: 0, level: 1, shards: 0 };
  return heroes;
}
...
/** Every hero you own, added up — what the hero screen means by "your power".
 *
 *  Distinct from rosterPower below, which counts only the cats standing on
 *  patrol. The screen header used rosterPower and so read "0 power" for a
 *  player with four heroes and nobody on duty, which is true of the patrol and
 *  false of the sentence it was in. */
export function ownedPower(save) {
  let p = 0;
  for (const [id, st] of Object.entries(save?.heroes || {})) {
    const hero = HERO_BY_ID[id];
    if (hero) p += heroPower(hero, st);
  }
  return Math.round(p);
}

/** The roster's raw power, for the raid — patrol only. */
