...
export function chapterState(chapter, save) {
  ...
  // A CHAPTER OPENS WHEN THE ONE BEFORE IT CLOSES.
  //
  // Kingshot's chapters arrive one at a time — that is the whole reason they
  // are chapters and not a list. Showing all of them at once turns a story
  // into a spreadsheet: a new player opens the book, sees forty tasks, and has
  // no idea which of them is theirs today. Locked chapters are still SHOWN,
  // with their name and the chapter that unlocks them, because a book you
  // cannot see the length of gives nothing to look forward to.
  const i = CHAPTERS.indexOf(chapter);
  const prev = i > 0 ? CHAPTERS[i - 1] : null;
  const locked = prev ? !claimed[prev.id] : false;
  ...
  return {
    ...
    complete: allClaimed,
    locked,
    unlockedBy: locked ? prev.name : null,
