...
import { BUILDING_BY_ID, COTTAGE_IDS, MAX_COTTAGES } from "./townConfig";
...
export const REQUIRES = {
  ...
  // in a specific way: every building the town unlocks becomes, from then on,
  ...
  //
  // That is what makes the crescendo real rather than cosmetic. A hall level is
  // not "wait for wood", it is "bring six buildings up with you", and each new
  // unlock quietly widens the web behind you.
  //
  // Nothing gates the FIRST level: the Storehouse and the Nap House only unlock
  // at Cat Hall 2, so requiring them earlier would deadlock a new save.
  hall: (level) => {
    if (level < 2) return [];
    const need = [
      { id: "storehouse", level: level - 1 },
      // The town centre cannot outgrow the town's own homes.
      { id: "cottage1", level: level - 1 },
    ];
    if (level >= 4) need.push({ id: "treats", level: level - 1 });
    if (level >= 6) need.push({ id: "garden", level: level - 2 });
    if (level >= 8) need.push({ id: "clinic", level: level - 2 });
    if (level >= 10) need.push({ id: "study", level: level - 2 });
    if (level >= 12) need.push({ id: "gatehouse", level: level - 2 });
    return need;
  },

  // Each producer feeds the building that stores or refines what it makes, so
  // no building is ever raised alone.
  storehouse: (level) => [{ id: "lumber", level }],
  treats: (level) => [{ id: "garden", level: Math.max(0, level - 2) }],
  adoption: (level) => [{ id: "treats", level }],
  quarry: (level) => [{ id: "lumber", level: Math.max(0, level - 1) }],
  garden: (level) => [{ id: "kitchen", level: Math.max(0, level - 1) }],

  // The late buildings lean on the mid ones, which lean on the first ones.
  watchtower: (level) => [{ id: "quarry", level }],
  clinic: (level) => [{ id: "garden", level }],
  study: (level) => [
    { id: "storehouse", level },
    { id: "quarry", level: Math.max(0, level - 1) },
  ],
  gatehouse: (level) => [
    { id: "quarry", level },
    { id: "watchtower", level: Math.max(0, level - 1) },
  ],
};
...
// is for. Written as a loop because eight identical rules are eight identical
...
export function villagerCap(levels = {}) {
  let beds = 0;
  for (const id of COTTAGE_IDS) beds += cottageBeds(levels[id] ?? 0);
  return Math.min(MAX_VILLAGERS, beds);
}

/** Villager places INSIDE one building. Every building comes with one, and up
 *  to two more can be bought — for that building alone. Capacity is bought
 *  where it is needed, not poured into a shared pool. */
...
/** Places in ONE building. Bought per building, never shared. */
export const slotsIn = (bought = 0) =>
  Math.min(MAX_PER_BUILDING, BASE_SLOTS_PER_BUILDING + bought);
 ...
 *  worth anything. */
const PER_GOLD = { wood: 70, fish: 70, treats: 60, stone: 18, catnip: 5 };
...
/** Collect a producer's store before it has filled. Priced on what is still
 *  missing from a full store, so a nearly-full building is nearly free. */
export function earlyCollectCost(pendingNow, cap) {
  const gap = Math.max(0, cap - pendingNow);
  return Math.max(1, Math.ceil(gap / 90));
}
