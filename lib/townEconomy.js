 ...
 *
 *  ONE bed to start. That is deliberate and it is the sharpest squeeze in the
 *  game: with a single worker the player can staff exactly one building and
 *  watches the other four crawl. The wall is unmissable inside the first
 *  session, which is the only time a wall is worth anything. Each Nap House
 *  level adds one, so going from 1 to 2 doubles the town's workforce and is
 *  felt immediately. */
export const workerCap = (napLevel) => napLevel;
...
export const REAL_PRICES = {
  builder: [null, null, 4.99, 14.99, 29.99],
  // The FIRST bought spot is the town's second worker — it doubles the
  // workforce, so it is a gateway purchase in its own right and priced like
  // one. The ladder climbs steeply after that.
  workerSlot: [4.99, 9.99, 19.99, 34.99, 49.99],
};
...
export function workerSlotPriceUsd(bought) {
  return REAL_PRICES.workerSlot[Math.min(bought, REAL_PRICES.workerSlot.length - 1)];
}
